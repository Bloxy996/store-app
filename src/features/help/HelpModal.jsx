...
const HELP_FEATURES = [
  ...
  { title: 'Vector editor', desc: 'A node/edge graph-based drawing tool (.vec file) — every line is a strict connection between two point Vertices, so moving a point deforms every line touching it. Tools: Vertex (click to place, click an existing line to insert a point on it), Edge (click two points in turn to connect them), Polyline (click to chain points, click the start point or press Enter to close/finish), Eyedropper (click a line to copy its color/thickness), and Flood fill (click an enclosed shape to color it in). Select tool: drag a box to select points, drag a point to move it and everything attached, Alt+drag a point to pull just one of its lines free instead of the whole point, G to group a multi-point selection (Shift+G to ungroup), double-click a grouped point to edit it individually without breaking the group. Export compiles the graph straight to a standalone SVG file.' },
