...
const HELP_FEATURES = [
  ...
  { title: 'Sparks', desc: 'Quick one-line captures for when you don\'t want to open a full note — type a line (or several; each line becomes its own spark) or attach a screenshot with a caption, file it under a nested category (e.g. vehicles/boat/small, autocompleted from ones you\'ve used before), and optionally link it to one or more notes/files (searches title, frontmatter, and content). Browse sparks by category in the Sparks panel, where you can also delete them; a note with linked sparks shows a small indicator at the bottom of reading view that jumps back to them.' },
  { title: 'Databases', desc: 'Notion-style structured tables stored as a .base file — table, board, gallery, calendar, chart, and timeline views, with typed columns (text, select, multi-select, date, image, etc). Calendar view needs a Date property (pick it from the view\'s settings panel) and places each row on the day its date falls on; click a day\'s + to add a row on that date. Chart view needs a Select property to group by, and switches between bar/line/pie from the icons above it — its settings panel also lets you switch from counting rows to summing or averaging a Number property per group. Timeline view needs a Start and an End date property, and draws a horizontal bar for every row that has both set — click a bar to open that row. Click a table column header to sort by it (ascending, then descending, then back to unsorted) for any sortable type. Click a view\'s chevron, or the + next to the view tabs, to expand its settings/add-view panel inline below the tabs.' },
  { title: 'Canvas', desc: 'An infinite, freeform board (.canvas file) for arranging notes, text cards, and images spatially and connecting them with arrows — pan by dragging the background (Shift+drag to box-select instead), scroll/pinch to zoom, drag a card\'s edge to draw a connection to another card. The grid-snap toolbar button (off by default) snaps cards to a grid when moved or resized.' },
  { title: 'Vector editor', desc: 'A node/edge graph-based drawing tool (.vec file) — every line is a strict connection between two point Vertices, so moving a point deforms every line touching it, and dropping a point onto a line binds it into that line instead of just sitting on top. Tools: Vertex (click to place, click an existing line to insert a point on it), Edge (click two points in turn to connect them), Polyline (click to chain points, click the start point or press Enter/Escape to finish), Eyedropper (click a line or a filled shape to copy its color), and Flood fill (click an enclosed shape to color it in — click an already-filled shape again to recolor it). A custom color swatch and a canvas background color are both in the toolbar. Select tool: drag a box to select points, drag anywhere inside the selection (or any selected point) to move the group, corner handles scale freely and the handles at the middle of each edge scale just that one axis, the handle above rotates, Alt+drag a point to pull just one of its lines free instead of the whole point. Nearby points/lines and shared axes highlight with a guide while dragging. Select 2+ points to Group them from the floating toolbar or with G (Shift+G or the Ungroup button to undo it); double-click a grouped point to edit it individually without breaking the group. Export compiles the graph straight to a standalone SVG file.' },
  { title: 'Bookmarks', desc: 'Star any note or file to pin it in the Bookmarks panel for quick access.' },
  { title: 'Available offline', desc: 'Use a file or folder’s … menu to make it available offline. A folder is a live rule: files added beneath it are cached automatically and files moved out stop being covered. Offline edits stay on this device until you reconnect and sync; changed or deleted Drive files always ask you how to resolve the conflict.' },
  ...
  { title: 'Graph view', desc: 'The network icon in the activity bar (or ⌘/Ctrl K → "Open graph view") opens the graph as its own tab, right alongside your notes — split it, close it, or Cmd-click to open it in a new tab like any note. Drag nodes, scroll or pinch to zoom, hover to see a note\'s connections, and click a node to jump straight to it. The sliders icon opens Filters (including a Tags toggle that adds every tag as its own node — click one to filter the sidebar search by that tag — and a Local graph toggle with a depth slider, showing just the notes connected to the one you had open), Groups (color notes matching text in their name), and Forces (repel/link/distance/center).' },
  ...
  { title: 'Frontmatter autocomplete', desc: 'With your cursor inside a note\'s frontmatter, an empty line suggests property names and a value suggests that property\'s options. Picking some values (e.g. type: game) auto-adds the properties that value implies. Fully customizable from the sliders icon in the sidebar footer — add/remove properties, edit their value list, and edit which properties a value adds.' },
  { title: 'Compile to XML', desc: 'The braces icon in the activity bar bundles a slice of your vault\'s notes into one XML document for pasting into an LLM chat for mass editing. Pick folders/notes to include or exclude (autocomplete included; leave Includes empty for the whole vault), Compile, then copy or download — the output ships with the exact reply format the LLM needs to follow. Paste or upload its XML reply back into the same panel\'s Apply section to write the changes to the store; a file only gets written if every one of its changes matches uniquely, and any file with unsaved edits open in an editor is skipped rather than overwritten.' }
];
